...
MAX_PLOTS = 30														# Absolute maximum number of plots, change if needed !!
COLORS = [x.split('#')[1] for x in sns.color_palette("cubehelix", MAX_PLOTS).as_hex()[:]]
...
class MyPlot(QWidget):
	...
	n_plots = MAX_PLOTS														# number of plots on the current plot. 
...
if __name__ == "__main__":
	...
	class MainWindow(QMainWindow):
		...
		def __init__(self):
			...
			self.plot.set_channels_labels(["E0", 
											"E1", 
											"E2"])
			...
			# self.data_timer.timeout.connect(lambda: self.on_data_timer(self.data))
